...
class AppConfigProvider(localStorage: ILocalStorage) {
    ...
    val fastpaydGrpcAuthority by lazy { Translator.getString(R.string.fastpaydGrpcAuthority) }
    val fastpaydGrpcUseTls by lazy {
        Translator.getString(R.string.fastpaydGrpcUseTls).toBooleanStrictOrNull() ?: true
    }
    val fastpaydLibp2pBootstrapPeers by lazy {
        Translator.getString(R.string.fastpaydLibp2pBootstrapPeers)
            .split(',')
            .map { it.trim() }
