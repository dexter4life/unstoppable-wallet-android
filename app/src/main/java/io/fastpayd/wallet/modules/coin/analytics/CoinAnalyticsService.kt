...
import io.fastpayd.wallet.core.InvalidAuthTokenException
import io.fastpayd.wallet.core.NoAuthTokenException
...
class CoinAnalyticsService(
    ...
    private suspend fun preview() {
        val addresses = accountManager.accounts.mapNotNull {
            it.type.evmAddress(App.evmBlockchainManager.getChain(BlockchainType.Ethereum))
        }

        try {
            val preview = marketKit.analyticsPreviewSingle(fullCoin.coin.uid, addresses).await()
