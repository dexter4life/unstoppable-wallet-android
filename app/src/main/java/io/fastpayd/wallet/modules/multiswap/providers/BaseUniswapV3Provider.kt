...
import io.fastpayd.wallet.modules.multiswap.EvmBlockchainHelper
import io.fastpayd.wallet.modules.multiswap.ISwapFinalQuote
import io.fastpayd.wallet.modules.multiswap.ISwapQuote
import io.fastpayd.wallet.modules.multiswap.SwapFinalQuoteEvm
import io.fastpayd.wallet.modules.multiswap.SwapQuoteUniswapV3
...
import io.fastpayd.wallet.modules.multiswap.settings.SwapSettingDeadline
...
abstract class BaseUniswapV3Provider(dexType: DexType) : IMultiSwapProvider {
    ...
    private fun uniswapToken(token: Token?, chain: Chain) = when (val tokenType = token?.type) {
        ...
        is TokenType.Eip20 -> uniswapV3Kit.token(
            io.fastpayd.ethereumkit.models.Address(
                tokenType.address
            ), token.decimals)
        else -> throw Exception("Invalid coin for swap: $token")
    }
