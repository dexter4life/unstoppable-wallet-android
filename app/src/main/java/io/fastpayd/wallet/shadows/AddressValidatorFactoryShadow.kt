...
object AddressValidatorFactory {
    fun get(token: Token): EnterAddressValidator {
        return when (token.blockchainType) {
            ...
            BlockchainType.Dash -> BitcoinAddressValidator(token, App.adapterManager)
            BlockchainType.Zcash -> ZcashAddressValidator(token, App.adapterManager)
            ...
            BlockchainType.ArbitrumOne -> EvmAddressValidator()
            BlockchainType.Solana -> SolanaAddressValidator()
            BlockchainType.Tron -> TronAddressValidator(token, App.adapterManager)
            BlockchainType.Ton -> TonAddressValidator()
            is BlockchainType.Stellar -> StellarAddressValidator(token)
            is BlockchainType.Unsupported -> EvmAddressValidator()
        }
    }
}